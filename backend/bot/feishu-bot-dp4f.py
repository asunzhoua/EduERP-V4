#!/usr/bin/env python3
"""
feishu-bot-dp4f.py — DP4F 飞书 Bot（lark-oapi WS 长连接模式）

独立 Bot 进程，通过 lark-oapi SDK 建立 WebSocket 长连接，
接收飞书消息事件，检查是否 @ 了当前机器人，如被 @ 则回复 "DP4F online"。

启动方式：直接运行 python feishu-bot-dp4f.py
凭证来源：自动从 DP4F 的 agent.json 读取
"""
...
log_handler = TimedRotatingFileHandler(
    os.path.join(LOG_DIR, "dp4f-bot.log"),
    when="midnight",
    backupCount=7,
    encoding="utf-8",
)
log_handler.setFormatter(logging.Formatter(
    "%(asctime)s [DP4FBot] %(levelname)s %(message)s"
))
...
logger = logging.getLogger("DP4FBot")
...
AGENT_JSON_PATH = os.path.join(
    os.path.expanduser("~"), ".qwenpaw", "workspaces", "DP4F", "agent.json"
)
...
logger.info(f"已加载 DP4F 飞书凭证: app_id={FEISHU_APP_ID[:10]}...")
...
def handle_message(event: P2ImMessageReceiveV1):
    ...
    # 被 @了，回复 "DP4F online"
    logger.info("✅ 被 @ 了，回复 'DP4F online'")
    reply_to_message(chat_id, message_id, "DP4F online")


def main():
    logger.info("=" * 60)
    logger.info("DP4F 飞书 Bot 启动中...")
