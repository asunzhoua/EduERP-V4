...
export class LessonExceptionController {
  ...
  async applySuspend(@Body() dto: ApplySuspendDto, @Req() req: AuthedRequest) {
    const operatorId = req.user.sub;
  ...
  async approve(
    ...
    if (user.role === 'Teacher') {
      const exception =
        await this.lessonExceptionService.findExceptionByIdWithRelations(id);
  ...
  @SwaggerResponse({
    status: 200,
    description: '成功返回异常详情（含课程、审批记录、补课信息）',
  })
  @SwaggerResponse({ status: 403, description: '无权访问' })
  ...
  async findOne(
    ...
    const logs =
      await this.lessonExceptionService.findExceptionsLogsByException(id);
    const reschedule =
      await this.lessonExceptionService.findRescheduleByExceptionId(id);
  ...
  async findReschedule(
    ...
    const result =
      await this.lessonExceptionService.findRescheduleByExceptionId(
        exceptionId,
      );
    return ApiResponse.success(result);
  }
