...
微信小程序结构：
miniapp/pages/ 包含登录页（login/）、首页仪表盘（index/）、教师端 7 个页面、学生端 3 个页面。
...
### 角色与权限
4 种角色：SuperAdmin / Admin / Teacher / Student。JWT 认证，Token 有效期 2 小时，支持 refresh token。教师端与学生端路由根据角色跳转。
